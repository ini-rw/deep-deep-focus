...
import { setSitesDataSync } from "../../../helpers/functions";
...
export function siteReducer(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case ADD_SITE:
      ...
      duplicate = state.allowedSites.find(
        (site) => site.name === action.payload.name
      );
      if (!duplicate) {
        updatedState = {
      ...
      return state;
    case REMOVE_SITE:
      updatedAllowedSite = state.allowedSites.filter(
